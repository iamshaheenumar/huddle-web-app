...
export default function SignupForm({ next }: { next?: string }) {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    const { error: authError } = await supabase.auth.signUp({
      email,
      password,
      options: { data: { display_name: displayName, group_name: groupName } },
    })
    if (authError) {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-3xl p-6" style={{ background: '#fff', border: '1px solid #F0ECE4' }}>
          ...
          <form onSubmit={handleSignup} className="flex flex-col gap-4">
            ...
            <div>
              <label className="text-xs font-bold mb-1.5 block" style={{ color: '#6B707A' }}>First group name</label>
              <input
                type="text"
                required
                value={groupName}
        ...
        <p className="text-center text-sm font-semibold mt-6" style={{ color: '#9A9FA8' }}>
          ...
          <Link href={next ? `/login?next=${encodeURIComponent(next)}` : '/login'} className="font-bold" style={{ color: '#3B6FF6' }}>Sign in</Link>
        </p>
      </div>
    </div>
  )
